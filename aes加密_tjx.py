...
# 列混淆（MixColumns）：对状态矩阵的每一列进行伽罗瓦域上的线性变换
def mix_columns(state):
    def gf_mult(a, b):  # 伽罗瓦域乘法，处理有限域GF(2^8)的运算
        p = 0
        for _ in range(8):
            if b & 1: p ^= a
            a <<= 1
            if a & 0x100: a ^= 0x1b  # 若高位溢出，使用模多项式x^8 + x^4 + x^3 + x + 1
            b >>= 1
        return p & 0xff
    s = [0] * 16
    for i in range(4):
...
# 逆列混淆（InvMixColumns）：对状态矩阵的每一列进行逆线性变换
...
# 轮密钥加（AddRoundKey）：将状态矩阵与轮密钥进行逐字节异或
...
class AESApp:
    def __init__(self, root):
        ...
        self.notebook.pack(fill='both', expand=True)  # 填充窗口
    ...
    def create_encrypt_ui(self):
        # 密钥长度选择
        tk.Label(self.encrypt_frame, text="选择密钥长度:").pack()
        self.encrypt_key_length = tk.StringVar(value="128")  # 默认128位
        tk.Radiobutton(self.encrypt_frame, text="128位（16字节）", variable=self.encrypt_key_length, value="128").pack()
        tk.Radiobutton(self.encrypt_frame, text="192位（24字节）", variable=self.encrypt_key_length, value="192").pack()
        tk.Radiobutton(self.encrypt_frame, text="256位（32字节）", variable=self.encrypt_key_length, value="256").pack()

        # 密钥输入框
        tk.Label(self.encrypt_frame, text="密钥:").pack()
        self.encrypt_key_entry = tk.Entry(self.encrypt_frame, width=40)
        self.encrypt_key_entry.pack()

        # 明文输入框
        tk.Label(self.encrypt_frame, text="明文:").pack()
        self.encrypt_input_text = tk.Text(self.encrypt_frame, height=10)
        self.encrypt_input_text.pack()

        # 加密按钮
        tk.Button(self.encrypt_frame, text="加密", command=self.encrypt).pack()

        # 密文输出框
        tk.Label(self.encrypt_frame, text="密文 (十六进制):").pack()
        self.encrypt_output_text = tk.Text(self.encrypt_frame, height=10)
        self.encrypt_output_text.pack()

    def create_decrypt_ui(self):
        # 密钥长度选择
        tk.Label(self.decrypt_frame, text="选择密钥长度:").pack()
        self.decrypt_key_length = tk.StringVar(value="128")  # 默认128位
        tk.Radiobutton(self.decrypt_frame, text="128位（16字节）", variable=self.decrypt_key_length, value="128").pack()
        tk.Radiobutton(self.decrypt_frame, text="192位（24字节）", variable=self.decrypt_key_length, value="192").pack()
        tk.Radiobutton(self.decrypt_frame, text="256位（32字节）", variable=self.decrypt_key_length, value="256").pack()

        # 密钥输入框
        tk.Label(self.decrypt_frame, text="密钥:").pack()
        self.decrypt_key_entry = tk.Entry(self.decrypt_frame, width=40)
        self.decrypt_key_entry.pack()

        # 密文输入框
        tk.Label(self.decrypt_frame, text="密文 (十六进制):").pack()
        self.decrypt_input_text = tk.Text(self.decrypt_frame, height=10)
        self.decrypt_input_text.pack()

        # 解密按钮
        tk.Button(self.decrypt_frame, text="解密", command=self.decrypt).pack()

        # 明文输出框
        tk.Label(self.decrypt_frame, text="明文:").pack()
        self.decrypt_output_text = tk.Text(self.decrypt_frame, height=10)
        self.decrypt_output_text.pack()
